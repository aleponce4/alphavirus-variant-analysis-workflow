...
# Trim
ivar trim -i "$BAM_TEST" -b $PRIMERS -p Ivar/$SAMPLE/trimmed \
    -q $IVAR_PRIMER_BASE_QUALITY -m $IVAR_PRIMER_WINDOW_SIZE -e 2>/dev/null || true

# Sort trimmed BAM
samtools sort -o Ivar/$SAMPLE/trimmed.sorted.bam Ivar/$SAMPLE/trimmed.bam 2>/dev/null || true
samtools index Ivar/$SAMPLE/trimmed.sorted.bam 2>/dev/null || true
...
ivar consensus -t 0.5 -m $IVAR_MIN_CONSENSUS_COVERAGE -n N -p Ivar/$SAMPLE/consensus 2>/dev/null || true

# Variants
samtools mpileup -aa -A -d 600000 -B -Q $IVAR_MIN_BASE_QUALITY -r $VIRAL_CONTIG -f $REFERENCE Ivar/$SAMPLE/trimmed.sorted.bam 2>/dev/null | \
ivar variants -t $IVAR_MIN_VARIANT_FREQ -m $IVAR_MIN_VARIANT_DEPTH -r $REFERENCE -g $ANNOTATION -p Ivar/$SAMPLE/variants 2>/dev/null || true

if [ -f "Ivar/$SAMPLE/variants.tsv" ]; then
    IVAR_COUNT=$(tail -n +2 Ivar/$SAMPLE/variants.tsv 2>/dev/null | wc -l || echo "0")
...
lofreq call -f $REFERENCE -r $VIRAL_CONTIG \
    ...
    "$BAM_TEST" > LoFreq/$SAMPLE/variants.vcf 2>/dev/null || true

if [ -f "LoFreq/$SAMPLE/variants.vcf" ]; then
    LOFREQ_COUNT=$(grep -v "^#" LoFreq/$SAMPLE/variants.vcf 2>/dev/null | wc -l || echo "0")
...
# Check annotations
echo -n "Annotations:  "
if [ -f "Annotated_variants/Ivar/${SAMPLE}_annotated.vcf" ] && [ -f "Annotated_variants/LoFreq/${SAMPLE}_annotated.vcf" ]; then
    echo "✓ both complete"
    ANNOT_OK=1
else
    echo "✗ FAILED"
...
if [ $IVAR_OK -eq 1 ] && [ $LOFREQ_OK -eq 1 ] && [ $ANNOT_OK -eq 1 ]; then
    ...
    echo ""
    echo "Files created:"
    echo "  - Ivar/$SAMPLE/variants.tsv (iVar variant calls)"
    echo "  - LoFreq/$SAMPLE/variants.vcf (LoFreq variant calls)"
    echo "  - Annotated_variants/Ivar/${SAMPLE}_annotated.vcf"
    echo "  - Annotated_variants/LoFreq/${SAMPLE}_annotated.vcf"
    echo ""
